    ...
    stages {
		...
		stage('docker build') {
            steps {
                sh 'docker build -t gcr.io/concise-haven-327611/pearson:${BUILD_NUMBER} .'
				...
				sh 'docker push gcr.io/concise-haven-327611/pearson:${BUILD_NUMBER} '
		...
		stage('kubernetes connect and deploy') {
            steps {
                ...
                sh "sed -i 's/00/${BUILD_NUMBER}/g' pearson-deploy.yml"
				sh 'kubectl apply -f pearson-deploy.yml'
				sh 'kubectl apply -f pearson-service.yml'
